...
def draw_overlay_v2(image, kept_segments, rejected_segments, k, r, t, title_lines, h_base_inv):
    out = image.copy()
...
def main():
    parser = argparse.ArgumentParser("Lane-line based road calibration v2 (lateral rejection + bounded roll)")
    ...
    parser.add_argument("--out-dir", default="outputs/calibration/road_line_v2")
    ...
    kept, rejected, fallback = filter_with_fallback(segments, h_base_inv, args.lat_thresh)
    baseline = score_calibration(kept, h_base_inv)
    ...
    for roll in roll_vals:
        for pitch in pitch_vals:
            for yaw in yaw_vals:
                ...
                cand = score_calibration(kept, h_cand_inv)
    ...
    left, _ = draw_overlay_v2(
        ...
        t_base,
        [
            "TASK: ROAD-LINE CALIBRATION v2",
            f"MODEL: {model_id} | CAM: {cam_id}",
            "LEFT: baseline (blue guides, white kept, red rejected, green fitted)",
            f"score={baseline['score']:.4f} kept={len(kept)} rejected={len(rejected)}",
            f"mean |dY/dX|={baseline['mean_abs_dy_dx']:.4f}",
        ],
        h_base_inv,
    )
    right, fitted_lines_ok = draw_overlay_v2(
        image,
        kept,
        rejected,
        k,
        ...
        [
            "TASK: ROAD-LINE CALIBRATION v2",
            f"MODEL: {model_id} | CAM: {cam_id}",
            "RIGHT: refined (blue guides, white kept, red rejected, green fitted)",
            f"score={best['score_info']['score']:.4f} kept={len(kept)} rejected={len(rejected)}",
    ...
    out_img = out_dir / f"{args.sample_id}_road_line_v2_compare.jpg"
    out_json = out_dir / f"{args.sample_id}_road_line_v2_result.json"

    cv2.imwrite(str(out_img), compare)

    personal_img = Path("personal-documents/calibration/images") / f"{args.sample_id}_road_line_calib_v2_anycalib_pinhole.jpg"
    personal_img.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(out_img, personal_img)

    result = {
        "sample_id": args.sample_id,
        "task": "road_line_calibration_v2",
        "model_id": model_id,
        "cam_id": cam_id,
        "model_tag": model_tag,
        "method": "lane-line orientation refinement with lateral rejection and bounded roll",
        ...
        "visual_legend": {
            "white": "kept longitudinal lane segments",
            "red_dim": "rejected lateral segments (crosswalk etc.)",
            "green": "fitted connected lane lines from kept clusters",
        ...
        "artifacts": {
            ...
            "personal_copy": str(personal_img.resolve()),
        },
    }
    ...
    print(f"Copied to: {personal_img.resolve()}")
    print(
        json.dumps(
            {
                "total_detected": len(segments),
